...
class ApiService extends GetxService {
  ...
  Future<List<Map<String, dynamic>>> getOrderedProducts(String accessToken) async {
    final url = Uri.parse('$baseUrl/api/orders/products'); // <-- Update this route
    final response = await http.get(
      url,
      headers: {
        'Authorization': 'Bearer $accessToken',
    ...
    if (response.statusCode == 200) {
      ...
      // Expected backend response: [{ "id": ..., "name": ... }, ...]
      return data.map<Map<String, dynamic>>((item) => {
        "id": item['id'].toString(),
        "name": item['name'] ?? "",
      }).toList();
    } else {
      throw Exception('Failed to fetch ordered products');
    }
  }

  Future<void> submitProdReview({
    String? productId, // null for general, id for product review
    required int rating,
    required String title,
    required String body,
    required String accessToken,
  }) async {
    final url = Uri.parse('$baseUrl/api/reviews'); // <-- Update this route
    final payload = {
      "rating": rating,
      "title": title,
      "body": body,
